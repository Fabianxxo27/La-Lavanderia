...
# -----------------------------------------------
# OPINIONES / TESTIMONIOS DEL CLIENTE
# -----------------------------------------------
@bp.route('/cliente_opiniones')
@login_requerido
def cliente_opiniones():
    """Ver y escribir opiniones del cliente."""
    username = session.get('username')
    if not username:
        return redirect(url_for('auth.login'))
    ...
    # Obtener opiniones del cliente
    mis_opiniones = run_query("""
        SELECT id_testimonio, calificacion, comentario, es_anonimo, aprobado,
               fecha_creacion, respuesta_admin
        FROM testimonio
        WHERE id_cliente = :ic
        ORDER BY fecha_creacion DESC
    """, {"ic": id_cliente}, fetchall=True)

    return render_template('cliente_opiniones.html', opiniones=mis_opiniones)


@bp.route('/cliente_enviar_opinion', methods=['POST'])
@login_requerido
def cliente_enviar_opinion():
    """Enviar una nueva opinión/testimonio."""
    username = session.get('username')
    if not username:
        return redirect(url_for('auth.login'))

    usuario = run_query(
        "SELECT u.id_usuario, u.nombre FROM usuario u WHERE LOWER(u.username) = :u",
        {"u": username.lower()},
        fetchone=True
    )
    if not usuario:
    ...
    comentario = limpiar_texto(request.form.get('comentario', '').strip(), 1000)
    es_anonimo = request.form.get('es_anonimo') == 'on'

    # Validaciones
    if not calificacion or calificacion < 1 or calificacion > 5:
        flash('Selecciona una calificación de 1 a 5 estrellas.', 'danger')
        return redirect(url_for('cliente.cliente_opiniones'))

    if not comentario or len(comentario) < 10:
        flash('El comentario debe tener al menos 10 caracteres.', 'danger')
        return redirect(url_for('cliente.cliente_opiniones'))

    nombre_publico = 'Anónimo' if es_anonimo else nombre_cliente

    run_query("""
        INSERT INTO testimonio (id_cliente, nombre_publico, calificacion, comentario, es_anonimo)
        VALUES (:ic, :np, :cal, :com, :anon)
    """, {
        "ic": id_cliente,
        "np": nombre_publico,
        "cal": calificacion,
    ...
    flash('¡Gracias por tu opinión! Será revisada por nuestro equipo.', 'success')
    return redirect(url_for('cliente.cliente_opiniones'))
