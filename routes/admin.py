...
# AGREGAR CLIENTE
# -----------------------------------------------
@bp.route('/registro-rapido', methods=['POST'])
@login_requerido
@admin_requerido
def registro_rapido():
...
@bp.route('/agregar_cliente', methods=['GET', 'POST'])
@login_requerido
@admin_requerido
def agregar_cliente():
...
# ELIMINAR CLIENTE
# -----------------------------------------------
@bp.route('/eliminar_cliente/<int:id_cliente>', methods=['POST'])
...
def ejecutar_migraciones_admin():
    ...
    archivos = ['add_direcciones_to_pedido.sql', 'create_descuento_config.sql', 'add_descuento_to_pedido.sql', 'create_cliente_esquema_descuento.sql', 'create_verification_codes.sql', 'alter_verification_codes_token.sql', 'create_notificaciones.sql', 'create_testimonios.sql']
    errores = []
    ...
    return redirect(url_for('admin.configurar_descuentos'))
...
# -----------------------------------------------
# OPINIONES / TESTIMONIOS (ADMIN)
# -----------------------------------------------
@bp.route('/admin/opiniones')
@login_requerido
@admin_requerido
def admin_opiniones():
    """Panel de administración de opiniones/testimonios."""
    filtro = request.args.get('filtro', 'todas')

    if filtro == 'aprobadas':
        where = "WHERE t.aprobado = TRUE"
    elif filtro == 'pendientes':
        where = "WHERE t.aprobado = FALSE"
    elif filtro == 'anonimas':
        where = "WHERE t.es_anonimo = TRUE"
    else:
        where = ""

    opiniones = run_query(f"""
        SELECT t.id_testimonio, t.nombre_publico, t.calificacion, t.comentario,
               t.es_anonimo, t.aprobado, t.clasificacion, t.respuesta_admin,
               t.fecha_creacion,
               CASE WHEN t.es_anonimo = FALSE AND t.id_cliente IS NOT NULL
                    THEN u.email ELSE NULL END as email_cliente,
               CASE WHEN t.es_anonimo = FALSE AND t.id_cliente IS NOT NULL
                    THEN c.telefono ELSE NULL END as telefono_cliente
        FROM testimonio t
        LEFT JOIN cliente c ON t.id_cliente = c.id_cliente
        LEFT JOIN usuario u ON t.id_cliente = u.id_usuario
        {where}
        ORDER BY t.fecha_creacion DESC
    """, fetchall=True)

    stats = {
        'total': run_query("SELECT COUNT(*) FROM testimonio", fetchone=True)[0],
        'aprobadas': run_query("SELECT COUNT(*) FROM testimonio WHERE aprobado = TRUE", fetchone=True)[0],
        'pendientes': run_query("SELECT COUNT(*) FROM testimonio WHERE aprobado = FALSE", fetchone=True)[0],
        'promedio': run_query("SELECT COALESCE(ROUND(AVG(calificacion)::numeric, 1), 0) FROM testimonio", fetchone=True)[0]
    }

    return render_template('admin_opiniones.html', opiniones=opiniones, stats=stats, filtro=filtro)


@bp.route('/admin/opinion/aprobar/<int:id_testimonio>', methods=['POST'])
@login_requerido
@admin_requerido
def aprobar_opinion(id_testimonio):
    """Aprobar o desaprobar un testimonio."""
    accion = request.form.get('accion', 'aprobar')
    aprobado = accion == 'aprobar'

    run_query(
        "UPDATE testimonio SET aprobado = :ap WHERE id_testimonio = :id",
        {"ap": aprobado, "id": id_testimonio}
    )

    flash(f'Opinión {"aprobada" if aprobado else "desaprobada"} correctamente.', 'success')
    return redirect(url_for('admin.admin_opiniones'))


@bp.route('/admin/opinion/clasificar/<int:id_testimonio>', methods=['POST'])
@login_requerido
@admin_requerido
def clasificar_opinion(id_testimonio):
    """Clasificar un testimonio."""
    clasificacion = limpiar_texto(request.form.get('clasificacion', '').strip(), 50)

    if clasificacion:
        run_query(
            "UPDATE testimonio SET clasificacion = :cl WHERE id_testimonio = :id",
            {"cl": clasificacion, "id": id_testimonio}
        )
...
@bp.route('/admin/opinion/responder/<int:id_testimonio>', methods=['POST'])
@login_requerido
@admin_requerido
def responder_opinion(id_testimonio):
    """Responder a un testimonio (enviar email al cliente si no es anónimo)."""
    respuesta = limpiar_texto(request.form.get('respuesta', '').strip(), 1000)
    ...
    # Guardar respuesta
    run_query(
        "UPDATE testimonio SET respuesta_admin = :resp WHERE id_testimonio = :id",
        {"resp": respuesta, "id": id_testimonio}
    )

    # Obtener datos del testimonio y cliente
    testimonio = run_query("""
        SELECT t.es_anonimo, t.comentario, t.calificacion, u.email, u.nombre
        FROM testimonio t
        LEFT JOIN usuario u ON t.id_cliente = u.id_usuario
        WHERE t.id_testimonio = :id
    """, {"id": id_testimonio}, fetchone=True)

    if testimonio and not testimonio[0] and testimonio[3]:
        # Enviar email si no es anónimo y tiene email
        email_cliente = testimonio[3]
        nombre_cliente = testimonio[4] or 'Cliente'
        cuerpo_html = f"""
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
            <div style="background: linear-gradient(135deg, #1a4e7b, #2d5f8d); padding: 20px; text-align: center;">
                <h1 style="color: #ffffff; margin: 0; font-size: 22px;">La Lavandería</h1>
                <p style="color: #a6cc48; margin: 5px 0 0; font-size: 14px;">Respuesta a tu opinión</p>
            </div>
            <div style="padding: 25px;">
                <p style="color: #333;">Hola <strong>{nombre_cliente}</strong>,</p>
                <p style="color: #555;">Gracias por compartir tu opinión con nosotros. Aquí está nuestra respuesta:</p>
                <div style="background: #f0f8e8; padding: 15px; border-radius: 8px; border-left: 4px solid #a6cc48; margin: 15px 0;">
                    <p style="color: #333; margin: 0;"><em>"{respuesta}"</em></p>
                </div>
                <p style="color: #999; font-size: 12px; margin-top: 20px;">Tu opinión original: "{testimonio[1][:100]}"</p>
            </div>
            <div style="background: #f8f9fa; padding: 15px; text-align: center;">
                <p style="color: #888; font-size: 12px; margin: 0;">La Lavandería — Su ropa en las mejores manos</p>
            </div>
        </div>
        """
        send_email_async(email_cliente, 'Respuesta a tu opinión — La Lavandería', cuerpo_html)
        flash('Respuesta guardada y email enviado al cliente.', 'success')
    else:
...
@bp.route('/admin/opinion/eliminar/<int:id_testimonio>', methods=['POST'])
@login_requerido
@admin_requerido
def eliminar_opinion(id_testimonio):
    """Eliminar un testimonio."""
    run_query(
        "DELETE FROM testimonio WHERE id_testimonio = :id",
        {"id": id_testimonio}
    )
