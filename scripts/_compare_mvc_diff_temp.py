...
for file_path, mvc_name, original_name in mvc_entries:
    ...
    if normalize(orig_block) != normalize(mvc_block):
        mismatches.append((file_path, mvc_name, original_name, normalize(orig_block), normalize(mvc_block)))
...
# Show diffs for first 3 mismatches
for entry in mismatches[:3]:
    file_path, mvc_name, original_name, orig_norm, mvc_norm = entry
    print('\n--- DIFF %s %s (original: %s) ---' % (file_path, mvc_name, original_name))
    diff = difflib.unified_diff(
        orig_norm.splitlines(),
        mvc_norm.splitlines(),
        fromfile='original:%s' % original_name,
        tofile='mvc:%s' % mvc_name,
        lineterm=''
    )
